...
const CSVReaderOriginal = () => {
    const { handleOriginalFileChanger, csvOriginal } = useDatos();
    ...
    return (
        <div className="px-4">
                ...
                <span className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-gray-600">
                        <span className="text-blue-600 underline no-underline text-2xl text-gray-500">
                            {" "}
                            Seleccione el archivo .CSV original
                        </span>
                    </span>
                    <input
                        type="file"
                        className="hidden"
                        onChange={handleOriginalFileChanger}
                    />
                </span>
            </label>
            ...
            <table className="table-auto w-full mt-4">
                <thead>
                    <tr>
                        {csvOriginal.length > 0 &&
                            Object.keys(csvOriginal[0]).map((header, index) => (
                                <th
                                    key={index}
...
export default CSVReaderOriginal;
